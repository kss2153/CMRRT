...
public class Graph : MonoBehaviour {
	...
	void createGraph() {
		...
		Vector3 rgb = hsb2rgb (currentRecords [0].color);
		...
		foreach (Experiment e in currentRecords [0].green_edges) {
			...
			GameObject node = createSphere ("neighbor_" + e.experiment_id, pos, c.transform);
			rgb = hsb2rgb (e.color);
			node.GetComponent<Renderer> ().material.color = new Color (rgb.x, rgb.y, rgb.z, 1f);
		...
		foreach (Experiment e in currentRecords [0].red_edges) {
			Vector3 rand = Random.insideUnitSphere.normalized;
			Vector3 pos = rand * 0.8f + center;
			GameObject node = createSphere ("neighbor_" + e.experiment_id, pos, c.transform);
			rgb = hsb2rgb (e.color);
