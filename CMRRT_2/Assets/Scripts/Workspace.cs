...
//        updateInfoDisplay ();
		...
		keyButton.onClick.AddListener (keyClick);
    }
	...
	public void keyClick() {
		if (keyShowing) {
			keyShowing = false;
			keyButton.gameObject.GetComponent<Image> ().color = Color.white;
			keyOverlay.SetActive (false);
		} else {
			keyShowing = true;
			keyButton.gameObject.GetComponent<Image> ().color = Color.red;
...
//		Debug.Log (temp);
	...
	public void updateInfoDisplay() {
		...
		if (ingredientsArray.Length > 0) {
			...
			SetRecords sr = transform.GetComponent <SetRecords>();

			outputText += "\n" + sr.matchedRecords.Count + " record(s) found | page " + sr.page_number + " of " + sr.max_page_number; 
