	...
	void fill_data ()
	{
		...
		foreach (Experiment e in records) {
			...
			if (!cleanedActivity.Equals ("") && allActivities.ContainsKey (cleanedActivity)) {
				allActivities.Add (cleanedActivity, allActivities[cleanedActivity] + 1);
			} else {
				allActivities.Add (cleanedActivity, 0);
			}
		}
			
		Color[] graphColors = {Color.blue, Color.cyan, Color.gray, Color.green, Color.magenta, Color.red, Color.yellow};
...
//		print (dict[new HashSet<Ingredient>() {beeswax, rosin}].Count);
	...
	void addTestData ()
	{
		foreach (Ingredient i in ingredients) {
			...
			foreach (Ingredient j in ingredients) {
				if (j.name != i.name) {
					records.Add (new Experiment (i.name + j.name, "SP2018", i.name + j.name, "", "kayvon", new Ingredient[]{ i, j }, i.color, "Experimenting", ""));
				}
			}
		}
	}
	...
	public string cleanActivity (string activity) {
		return Regex.Replace (activity.ToLower (), "[^A-Za-z0-9 _]", "");
	}
